            ...
            <div className=" w-full items-center justify-center flex flex-col gap-6 mb-4 ">
                {repositoryDetails.map((repo, index) => (
                    <div key={repo.id}
                        ...
                        className={`w-11/12 none gap-3 p-4 flex-col ${isProfessionalProject ? '' : 'cursor-pointer'} bg-cyan-200 rounded-2xl shadow-md ${isProfessionalProject ? '' : 'hover:bg-sky-300 hover:scale-105 transition ease-in-out delay-75'}  flex`}
                    ><div className="flex  w-full justify-between">
                            <p className="uppercase font-medium">{repo.name}</p>
                            <p>{formatDate(repo.created_at)}</p>
                        </div>
                        <p >{repo.description}</p>
                        {repo.homepage && <div className="bg-teal-400 hover:bg-teal-600 hover:text-white transition ease-in-out hover:scale-105 w-fit break-all  p-2 gap-1  rounded-2xl" onClick={() => window.open(repo.homepage, '_blank', 'noreferrer')}> {repo.homepage}</div>}
                        <p className="uppercase font-medium">{repo.language}</p>
                        <div className="grid grid-cols-2 lg:grid-cols-3 text-center gap-2">
                            {repo.topics.map((topic) => <div key={topic} className="bg-teal-400 animate-appear-1 break-words capitalize  p-2 gap-1  rounded-2xl" > {topic}</div>)}
                        </div>
                    </div>
                ))
                }
