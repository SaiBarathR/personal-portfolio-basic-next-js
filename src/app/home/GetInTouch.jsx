...
export default function GetInTouchDrawer({ toggleDrawer, open }) {
    ...
    return <div className={" w-full bg-teal-50 md:w-auto min-h-[200px] flex flex-row  h-full dark:bg-gradient-to-r from-emerald-500 from-40% via-sky-400 via-100% " + space_grotesk.className}>
        <div className="w-[8px] h-[38px] bg-slate-400  dark:bg-black rounded-lg flex self-center items-center m-2 cursor-pointer " onMouseDown={toggleDrawer(false)}></div>
...
export function GeitInTouchContent({ open }) {
    ...
    return <div className={`w-full flex flex-col bg-cyan-100 dark:bg-gradient-to-r from-emerald-500 from-20% via-sky-500 via-100% shadow-2xl p-4 md:p-6  gap-5 items-start rounded-2xl mb-5 overflow-y-auto ${open ? "animate-appear" : "animate-disappear"}`}>
